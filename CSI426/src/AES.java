...
public class AES {
 ...
 //Added by Wyatt:
 private byte[] iv; //fixed-sized input to cryptographic primitives for randomness, starting point
...
Because of this difference in processing, separate methods (encryptCBC and decryptCBC) are required for encryption and decryption in CBC mode.
 ...
 public byte[] encryptCBC(byte[] plaintext) {
     if (iv == null || iv.length != 16) {
         throw new IllegalStateException("IV must be 16 bytes long.");
     }
 ...
 public byte[] decryptCBC(byte[] ciphertext) {
     ...
     // Padding Removal
     byte lastByte = decrypted[decrypted.length - 1];
     int paddingLength = (int) lastByte;
     if (paddingLength < 1 || paddingLength > 16) {
         throw new IllegalStateException("Invalid padding length.");
     }
...
This symmetry allows us to use a single method
  ...
  */
 public byte[] encryptDecryptOFB(byte[] input) {
